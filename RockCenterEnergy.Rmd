...
A look at three years of electric and steam consumption at Rockefeller Center using just annual consumptions. We capture the Covid 2019 lock down which was imposed March of 202o to compare with the following 2 years. All values in kWh and M pounds of steam to kBTU's for both. I would have expected a greater increase in 2021 but it is close to the same as 2020. Overall 2020 is `r CampusDelta[2,4]` of 2019 and 2021 is `r CampusDelta[3,4]` of 2019. 
...
group_by(Property, "Year" = year(Date)) %>%  #"Month" = month(Date)) %>% 
  summarise(Population = sum(Total.Entrants)) -> Turnstile_data
Turnstile_data$Year <- as.factor(Turnstile_data$Year)
